...
class FreScore:
    ...
    def get_score(self):
        # bart_scores = []
        if self.summary_column + '_' + 'auto_eval_fre' in self.df.columns:
            rows_to_process = self.df[self.df[self.summary_column + '_' + 'auto_eval_fre'].isna()]
        else:
        ...
        for row_index, (summary, input_text) in enumerate(zip(rows_to_process[self.summary_column], rows_to_process[self.article_column])):
            ...
            print('fre score', row_index, score)
            self.df = update_and_save_df(self.df, rows_to_process.index[row_index], self.summary_column + '_' + 'auto_eval_fre', score, self.output_file_path)
